...
function isFalse (s) {
  // [ilg] return typeof s !== 'number' && !s
  // This should fix empty attributes.
...
XMLWriter.prototype = {
  ...
  startDocType: function (name, pubid, sysid, subset) {
    ...
    if (!name.match(this.name_regex)) throw Error('Invalid Parameter')
    if (pubid && !pubid.match(/^[\w-][\w\s\-/+:.]*/)) {
      throw Error('Invalid Parameter')
    }
    if (sysid && !sysid.match(/^[\w.][\w\-/\\:.]*/)) {
      throw Error('Invalid Parameter')
    }
    if (subset && !subset.match(/[\w\s<>+.!#\-?*,()|]*/)) {
      throw Error('Invalid Parameter')
    }
